    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-6">
      ...
      <div class="flex items-center gap-4">
        <input 
          ...
          type="date" 
          class="bg-white border border-slate-200 rounded-xl px-4 py-2.5 text-sm font-bold text-primary outline-none focus:ring-2 focus:ring-accent/20 focus:border-accent transition-all shadow-sm"
        >
        <button 
          @click="saveAttendance" 
          :disabled="isSaving"
          class="btn-primary shadow-xl shadow-primary/20 disabled:opacity-50"
        >
      ...
      <div class="overflow-x-auto">
        <table class="w-full text-left border-collapse">
          ...
          <tbody class="divide-y divide-slate-50">
            ...
            <tr v-else v-for="emp in employees" :key="emp.id" class="hover:bg-slate-50/30 transition-colors group">
              <td class="px-6 py-5">
            ...
            <tr v-if="!isLoading && employees.length === 0">
              <td colspan="5" class="px-6 py-20 text-center">
                <i class="pi pi-users text-4xl text-slate-200 mb-4"></i>
